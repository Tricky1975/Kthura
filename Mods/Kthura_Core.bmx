...
MKL_Version "Kthura Map System - Kthura_Core.bmx","17.04.09"
...
Rem
bbdoc: If set "True" this variable will deem a WalkTo request succesful if the actor is already on the spot. If this variable is False, then the WalkTo request will very likely return False.
End Rem
Global OnSpotSuccess = True
...
Type TKthuraActor Extends TKthuraObject
	...
	Rem
	bbdoc: Checks if an actor is on the coordinates. For this feature the Actor does not have to be on the same X and Y position as the required coordinates, but rather in the same "tile" generated by the BlockMapGrid.
	...
	Rem
	bbdoc: Checks if an actor is on the spot. For this feature the Actor does not have to be on the same X and Y position as the required spot, but rather in the same "tile" generated by the BlockMapGrid.
	returns: True if the actor is on the spot, False if the actor is not.
	End Rem
